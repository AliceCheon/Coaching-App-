  ...
  function createDecisionEngine(options={}){
    ...
    function mergeSimilarDecisions(decisions){
      ...
      list(decisions).forEach(decision=>{
        const target=decision.exerciseId||decision.sheetId||"program",key=`${decision.category||"general"}|${target}`,current=groups.get(key);
