...
const result = vm.runInContext(`(() => {
  ...
  const customId=window.BarbellDivaMasterLibrary.stableId("custom","Glute Drive Alice"), deleteId=window.BarbellDivaMasterLibrary.stableId("custom","Custom da eliminare");
  check("TEST 3 creazione custom",custom.id===customId&&custom.isCustom);
  const snapshot=JSON.parse(JSON.stringify(state)); state=mergeState(clone(baseState),snapshot);
  check("TEST 4 custom dopo refresh",technicalExerciseLibrary().some(x=>x.id===customId));
  coachProgramUi.programId=program.id; coachProgramUi.sheetId=sheet.id;
  ...
  check("TEST 9 ricerca e filtri",exerciseLabFilteredProfiles().some(x=>x.id===customId));
  ...
  const analyzedManual=upsertTechnicalExercise(analyzeTechnicalExercise(manual)); const refreshedManual=technicalExerciseLibrary().find(x=>x.id===manual.id);
