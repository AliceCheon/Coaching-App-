...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14756"]) {
...
check(config.includes('build: "v147.56-flexwindow-fullbleed"') && config.includes('cache: "atlas-app-v14756-flexwindow-fullbleed"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14756-flexwindow-fullbleed"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14756"), "manifest non allineato alla build v147.22-sync-pwa-forced");
