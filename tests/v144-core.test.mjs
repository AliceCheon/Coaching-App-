...
check(config.includes('build: "v147.1-intensita-persistence"') && config.includes('cache: "atlas-app-v1471-intensita-persistence-fix"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v1471-intensita-persistence-fix"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v1471"), "manifest non allineato alla build v147.1");
