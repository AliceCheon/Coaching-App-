...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14761"]) {
...
check(config.includes('build: "v147.61-canvas-no-fixed-bg"') && config.includes('cache: "atlas-app-v14761-canvas-no-fixed-bg"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14761-canvas-no-fixed-bg"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14761"), "manifest non allineato alla build v147.22-sync-pwa-forced");
