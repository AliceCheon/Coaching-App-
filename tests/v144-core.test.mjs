...
check(config.includes('build: "v147.14-syntax-fix"') && config.includes('cache: "atlas-app-v14714-syntax-fix"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14714-syntax-fix"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14714"), "manifest non allineato alla build v147.9");
