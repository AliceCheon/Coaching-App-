...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14744"]) {
...
check(config.includes('build: "v147.44-coach-ai-sync"') && config.includes('cache: "atlas-app-v14744-coach-ai-sync"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14744-coach-ai-sync"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14744"), "manifest non allineato alla build v147.22-sync-pwa-forced");
