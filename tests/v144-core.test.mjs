...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14752"]) {
...
check(config.includes('build: "v147.52-fase-da-dati-programma"') && config.includes('cache: "atlas-app-v14752-fase-da-dati-programma"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14752-fase-da-dati-programma"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14752"), "manifest non allineato alla build v147.22-sync-pwa-forced");
