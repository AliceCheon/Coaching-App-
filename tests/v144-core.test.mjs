...
check(config.includes('build: "v147.15-sync-unlock"') && config.includes('cache: "atlas-app-v14715-sync-unlock"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14715-sync-unlock"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14715"), "manifest non allineato alla build v147.9");
