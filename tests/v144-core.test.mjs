...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14760"]) {
...
check(config.includes('build: "v147.60-cutout-edge-to-edge"') && config.includes('cache: "atlas-app-v14760-cutout-edge-to-edge"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14760-cutout-edge-to-edge"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14760"), "manifest non allineato alla build v147.22-sync-pwa-forced");
