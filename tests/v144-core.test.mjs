...
check(config.includes('build: "v147.21-sync-boot-persist"') && config.includes('cache: "atlas-app-v14721-sync-boot-persist"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14721-sync-boot-persist"'), "cache service worker non allineata alla build v147.1");
