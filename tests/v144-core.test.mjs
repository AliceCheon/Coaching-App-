...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14750"]) {
...
check(config.includes('build: "v147.50-fase-scheda-label"') && config.includes('cache: "atlas-app-v14750-fase-scheda-label"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14750-fase-scheda-label"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14750"), "manifest non allineato alla build v147.22-sync-pwa-forced");
