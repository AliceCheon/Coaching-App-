...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14751"]) {
...
check(config.includes('build: "v147.51-scheda-prima-fase-derivata"') && config.includes('cache: "atlas-app-v14751-scheda-prima-fase-derivata"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14751-scheda-prima-fase-derivata"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14751"), "manifest non allineato alla build v147.22-sync-pwa-forced");
