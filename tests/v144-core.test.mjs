...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14755"]) {
...
check(config.includes('build: "v147.55-blocco-settimana"') && config.includes('cache: "atlas-app-v14755-blocco-settimana"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14755-blocco-settimana"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14755"), "manifest non allineato alla build v147.22-sync-pwa-forced");
