...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14759"]) {
...
check(config.includes('build: "v147.59-edge-to-edge"') && config.includes('cache: "atlas-app-v14759-edge-to-edge"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14759-edge-to-edge"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14759"), "manifest non allineato alla build v147.22-sync-pwa-forced");
