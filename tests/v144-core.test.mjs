...
for (const marker of ["queueReliableWorkoutSession", "reliableSyncQueue.enqueue", "flushReliableSync", "reconcileSessionVersions", "sync-reliability.js?v=v14762"]) {
...
check(config.includes('build: "v147.62-short-edges-fullscreen"') && config.includes('cache: "atlas-app-v14762-short-edges-fullscreen"'), "configurazione v147.1 non caricata correttamente");
check(sw.includes('const CACHE_NAME = "atlas-app-v14762-short-edges-fullscreen"'), "cache service worker non allineata alla build v147.1");
check(manifest.includes("index.html?v=v14762"), "manifest non allineato alla build v147.22-sync-pwa-forced");
