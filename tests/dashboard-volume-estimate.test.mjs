...
const result = vm.runInContext(`(() => {
  ...
  const todaySession = structuredClone(july[july.length - 1]);
  todaySession.id = "sessione-oggi";
  todaySession.dateInput = todayIso;
  todaySession.date = new Date().toLocaleDateString("it-IT");
  todaySession.sessionCode = "OGGI";
  todaySession.sessionName = "Scheda OGGI";
  state.training.sessions = [...state.training.sessions, todaySession];
  const stats = dashboardWorkoutVolume();
