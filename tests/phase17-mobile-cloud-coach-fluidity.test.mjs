...
const result = await vm.runInContext(`(async()=>{
  ...
  const programDocs=[...collections.nutritionPhotos.values()];
  const programIds=new Set(programDocs.map(item=>item.programId).filter(Boolean));
  if(programIds.size!==state.programs.length) throw new Error("Programmi non separati nel cloud");
  ...
  const remoteSheetRecord=programDocs.find(item=>item.programId===program.id&&item.sheetId===sheet.id);
  const remoteExercise=remoteSheetRecord.sheet.exercises.find(e=>e.id===exercise.id);
